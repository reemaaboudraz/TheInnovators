...
describe("shouldShowBuildingLabel", () => {
    ...
    it("shows big buildings (z=1) when maxDelta <= 0.05", () => {
        expect(shouldShowBuildingLabel(makeBuilding(1), makeRegion(0.05, 0.01))).toBe(
            true,
        );
        expect(shouldShowBuildingLabel(makeBuilding(1), makeRegion(0.051, 0.01))).toBe(
            false,
        );
    });

    it("shows medium buildings (z=2) when maxDelta <= 0.02", () => {
        expect(shouldShowBuildingLabel(makeBuilding(2), makeRegion(0.02, 0.01))).toBe(
            true,
        );
        expect(shouldShowBuildingLabel(makeBuilding(2), makeRegion(0.021, 0.01))).toBe(
            false,
        );
    });
